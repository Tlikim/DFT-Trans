...
def main(args):
    ...
    if args.do_train:
        ...
        # bert_config_path = os.path.join(args.model, "config.json")
        # config = BertConfig.from_pretrained("/root/autodl-tmp/our_bert_chinese", num_labels=2)
        config = AlbertConfig.from_pretrained("/root/autodl-tmp/our_bert_chinese", num_labels=2)
        for lr in lrs:
            model = BertForQuestionAnswering(config)
            model.model.load_state_dict(torch.load("/root/autodl-tmp/our_bert/origina.bin"))
            if args.fp16:
                model.half()
            model.to(device)
            if n_gpu > 1:
            ...
            for epoch in range(int(args.num_train_epochs)):
                ...
                for step, batch in enumerate(tqdm(train_batches)):
                    ...
                    loss = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, start_positions=start_positions, end_positions=end_positions)
                    if n_gpu > 1:
                        loss = loss.mean()
                    if args.gradient_accumulation_steps > 1:
    ...
    if args.do_eval:
        ...
        # model = BertForQuestionAnswering(in_chans=768,num_classes=2, num_stages=5, embed_dims=768, stem_hidden_dim=768,drop_path_rate=0.1,num_heads=4,norm_layer=partial(nn.LayerNorm,eps=1e-12), token_label=False,mlp_ratios=[2,2,1], vocab_size=30522)
        loaded_paras = torch.load(os.path.join(args.output_dir, WEIGHTS_NAME))
        model.model.load_state_dict(loaded_paras)
        if args.fp16:
            model.half()
        model.to(device)
